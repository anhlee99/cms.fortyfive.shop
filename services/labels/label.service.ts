import type { Label, CreateLabelDTO, LabelSearchParams, LabelType} from "./label.type";
...
export async function listAllLabelsByType(labelType: LabelType): Promise<Label[]> {
  const supabase = await createClient();
...
export async function update(id: string, input: Partial<CreateLabelDTO>): Promise<Label> {
    const supabase = await createClient();
    const { data, error } = await supabase
        .from("labels")
        .update({ ...input })
